...
%----First of four subplots.---------------------------------------
  ...
  axis([-3 3 -3 3 -2 1]);
  view(-15,55);
  title('Michaelwicz function');
  hold on;
  plot3(S_struct.FM_pop(:,1),S_struct.FM_pop(:,2),...
     -(sin(S_struct.FM_pop(:,1)).*(sin((S_struct.FM_pop(:,1).*S_struct.FM_pop(:,1))/pi)).^20 +...
       sin(S_struct.FM_pop(:,2)).*(sin(4*(S_struct.FM_pop(:,2).*S_struct.FM_pop(:,2))/pi)).^20),...
       'r.');
  hold off;
